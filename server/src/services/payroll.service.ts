...
const processPayroll = async ({
    ...
    try {
        ...
        if (paymentType === 'salary') {
            ...
            const staffShiftIds = allAssignments.map((sa) =>
                sa.shiftId._id.toString(),
            );
            ...
            daysInMonth.forEach((day: Date) => {
                ...
                // 1. Check unemployed
                const isBeforeJoin =
                    joinDate && dayStr < getBDDateString(joinDate);
                const isAfterExit =
                    exitDate && dayStr > getBDDateString(exitDate);
                if (isBeforeJoin || isAfterExit) {
                    unemployedDays++;
                    return;
                }
            ...
            // Half-day and late count as present with full payment
            // LOGIC REFINEMENT: totalDeductionUnits capped at 30 to prevent >100% deduction in 31-day months.
            const totalDeductionUnits = Math.min(
                30,
                absentDays + unemployedDays,
            );
        ...
        const currentFinalAmount =
            await analyticsService.getCurrentFinalAmount(session);
        ...
        if (existingExpense) {
            ...
            if (amount > maxAllowed) {
                throw new Error(
                    'Insufficient balance. Expense exceeds available amount.',
                );
            }
...
const bulkProcessPayment = async ({
    ...
    try {
        ...
        const totalBulkAmount = payments.reduce(
        ...
        for (const payment of payments) {
            const staff = staffs.find(
                (s) => s._id.toString() === payment.staffId,
            );
...
const graceAttendance = async (
    ...
    const monthStr = getBDDateString(firstDate)
        .split('-')
        .slice(0, 2)
        .join('-');
    ...
    for (const date of dates) {
        ...
        if (attendance) {
            ...
            const dayAssignment = (allAssignments as any[]).find((sa) => {
                ...
                const e = saEnd ? getBDDateString(saEnd) : '9999-12-31';
                return dayStr >= s && dayStr <= e!;
            });
            ...
            if (bulkExpense) {
                ...
                if (match && match[1]) {
                    ...
                    if (filteredPairs.length === 0) {
                        ...
                        const staffObj =
                            await StaffModel.findById(staffId).session(session);
                        const staffName = staffObj
                            ? (staffObj as any).name
                            : '';
                        ...
                        bulkExpense.amount = Math.max(
                            0,
                            bulkExpense.amount - individualAmount,
                        );
...
async function setAttendance(payload: {
    ...
    if (existing) {
        ...
        existing.notes =
            (existing.notes || '') + ' | Manually updated via Payroll Calendar';
