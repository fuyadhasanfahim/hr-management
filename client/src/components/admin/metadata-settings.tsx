...
import { Loader2, Plus, Trash2, Building2, Briefcase, Tag } from 'lucide-react';
    ...
    return (
        <Card>
            ...
            <CardContent>
                <Tabs
                    value={activeTab}
                    onValueChange={(v) => setActiveTab(v as 'department' | 'designation')}
                >
...
function MetadataList({ type }: { type: 'department' | 'designation' }) {
    ...
    const [createMetadata, { isLoading: isCreating }] = useCreateMetadataMutation();
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            toast.success(`${type === 'department' ? 'Department' : 'Designation'} deleted`);
        } catch (err: any) {
    ...
    const typeNamePlural = type === 'department' ? 'departments' : 'designations';
    ...
    return (
        <div className="space-y-4">
            ...
            <form onSubmit={handleAdd} className="flex gap-2">
                ...
                <Button type="submit" disabled={isCreating || !newLabel.trim()} className="gap-2 h-10">
                    {isCreating ? (
            ...
            <div className="rounded-lg border">
                {items.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="text-muted-foreground font-medium">No {typeNamePlural} yet</p>
                            ...
                            >
                                ...
                                <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                        <AlertDialogHeader>
                                            <AlertDialogTitle>
                                                Delete {type === 'department' ? 'Department' : 'Designation'}?
                                            </AlertDialogTitle>
                                            <AlertDialogDescription>
            ...
            {items.length > 0 && (
                <p className="text-sm text-muted-foreground text-center">
                    {items.length} {items.length === 1 ? typeName : typeNamePlural}
                </p>
            )}
